...
describe('Mock Functions', () => {
  ...
  describe('usingCallback', () => {
    ...
    it('callback should be executed 4 times (one for each element in array)', () => {
      ...
      expect(mockFunction.mock.calls.length).toBe(4); //vemos cuantas veces se llamo al cb
    });

  });

  xdescribe('cacheFunction', () => {
    ...
    it('callback should not be executed again if already called with that arg', () => {
      ...
      cacheSum10(1);
      cacheSum10(7);
      expect(mockFunction.mock.calls.length).toBe(3); //vemos cuantas veces se llamo al cb
    });
