...
function problemC () {
  ...
  // ???
  promisifiedReadFile('poem-one/stanza-02.txt').then(data2 => {
    blue(data2); 
    return promisifiedReadFile('poem-one/stanza-03.txt')}).then(data3 => blue(data3))
}
...
function problemD () {
  ...
  promisifiedReadFile('poem-one/wrong-file-name.txt')
    .then(data => blue(data)) // la otra opcion es meter el error como segundo parametro del .them , (err) =>  magenta(new Error(err))
